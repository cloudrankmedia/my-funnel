const BENEFITS = [
  {
    ...
    icon: (
      <svg className="w-5 h-5 stroke-brand-orange" fill="none" viewBox="0 0 24 24" strokeWidth={1.5}>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {BENEFITS.map(({ title, copy, icon }) => (
            <div
              key={title}
              className="bg-white rounded-xl p-6 border border-gray-100 hover:border-brand-orange/30 transition-colors"
            >
              <div className="w-10 h-10 bg-brand-orange-pale rounded-lg flex items-center justify-center mb-4">
                {icon}
              </div>
              <h3 className="text-lg font-medium text-gray-900 mb-2">{title}</h3>
              <p className="text-base text-gray-500 leading-relaxed">{copy}</p>
            </div>
          ))}
        </div>
      </div>
